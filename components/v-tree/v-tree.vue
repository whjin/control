      ...
      <div class="uni-tree-item" :key="getId(item.id)">
        <div class="item-wrapper">
          <div class="item-name-box" :class="
            ...
            <div :class="{ 'select-box-disabled': disabledSelect }" v-if="isShowCheckbox(item)" @click="toggleCheck(currentLevelData[index])">
              <common-icons :type="item._checked ? 'iconcheckbox' : 'iconcheck-unselect'" color="#2A4273" size="24"></common-icons>
            </div>
            <div :class="{ 'select-box-disabled': disabledSelect }" v-if="isShowRadio(item)" @click="toggleSelect(currentLevelData[index])">
              <common-icons :type="item._selected ? 'iconcheckbox' : 'iconcheck-unselect'" color="#2A4273" size="24"></common-icons>
            </div>
            <div :class="{ 'select-box-disabled': disabledSelect && level != 0 }">
              <div class="item-title" :class="level == 0 ? 'item-name-active' : 'item-title'" @click="selectLabel(currentLevelData[index], index)">
                {{ item.name }}
              </div>
            </div>
