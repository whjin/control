            ...
            <div v-if="page == 1" class="groups-list">
              ...
              <div class="terminal-room-box">
                <scroll-view scroll-y="true">
                  <v-tree ref="vTree" showCheckbox :showGroup="true" :root="groupList" :changeHandler="groupSelect" @group-change="openModifyGroup" @group-delete="openDeleteGroup"></v-tree>
                </scroll-view>
              </div>
            </div>
          </div>
      ...
      <div class="control-center">
        <div class="control-center-head">
          <div class="center-head-box">
            <div class="head-tabs" v-for="(item,idx) in controlList" :key="item.index">
              <div :class="currentPage == item.index ? 'tab-active bottom-tab-active' : 'tab'" @click="handlePageChange(item.index)">
                <common-icons :type="item.type" :color="currentPage == item.index ? '#35fffa' : '#fff'" size="24"></common-icons>
                {{item.name}}
              </div>
            </div>
          </div>
          ...
          <div class="control-center-content">
            <div class="center-btn-box">
              ...
              <div class="center-btn-right" style="font-size: 18upx;" @click="showAlarmModal">
                已选中：<text style="color: #21cdc5;font-size: 18upx;">{{ selectSongNum }}</text>
              </div>
            </div>
                  ...
                  <scroll-view scroll-y="true" class="control-table-scroll">
                    <div class="control-table-main" v-for="(item, index) in audioTableList" :key="index">
                      <div class="table-content">
                        ...
                        <div class="control-table-item" style="flex: 1" :class="{red: currentPage === 2 && item.status}">
                          {{ currentPage > 4 ? item.status ? '在线' : '离线' : item.status ? currentPage === 2 ? '报警中' : '已开启' : '已关闭' }}
                        </div>
                        ...
                        <div class="control-table-item" style="flex: 1">
                          <common-icons v-if="currentPage <= 4" :type="type" :color="item.status ? currentPage === 2 ? 'red' : '#21cdc5' : '#ccc'" size="24" @click="handleItemChange(item, !item.status)"></common-icons>
                          <template v-else>
                            <button type="primary" style="margin-left: 5upx; margin-right: 5upx;" @click="readingDate(index)">读取</button>
                            <button type="primary" style="margin-left: 5upx; margin-right: 5upx;" @click="handleItemChange">上传</button>
                          </template>
                        </div>
                      </div>
            ...
            <div class="right-info-list">
              <div v-for="(pitem, pindex) in prisonerList" :key="pitem.id">
                <div class="right-info-item" :style="pindex % 2 ? 'margin-left: 16.66upx' : ''">
                  ...
                  <div class="info-img">
                    <div class="content">名称：<span>{{ pitem.xm }}</span></div>
                  </div>
                  <div class="info-img">
                    <div class="content">犯号：<span>{{ pitem.dabh.slice(-4) }}</span></div>
                  </div>
                  <div class="info-img">
                    <div class="content">床位号：<span>{{ pitem.cwh }}</span></div>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="neil-modal-container">
      ...
      <neil-modal :show="showAddGroup" @close="closeModal('AddGroup')">
        <div class="control-modal-container" style="width: 480upx; height: 320upx;">
          <div class="neil-modal-label">
            确认{{ currentPage > 4 ? isRead ? '读取' : '上传' : addGroupName ? '开启' : '关闭'}}？
          </div>
          <div class="neil-modal-btn">
...
import controlList from "@/static/mock/controlList.json";
import controlColumns from "@/static/mock/controlColumns.json";
import roomColumns from "@/static/mock/roomColumns.json";
import doorColumns from "@/static/mock/doorColumns.json";
import temColumns from "@/static/mock/temColumns.json";
import otherColumns from "@/static/mock/otherColumns.json";
import controlTableList from "@/static/mock/controlTableList.json";
...
export default {
  ...
  methods: {
    ...
    async getTerminalInfo () {
      ...
      let res = await Api.apiCall("get", Api.index.getRoomByAreaId, { id: areaId });
    ...
    async getGroupList () {
      ...
      let res = await Api.apiCall("get", Api.index.getGroupList, { controlId: controlId });
      if (res.state.code == "200") {
        let result = [];
        res.data.map(item => {
          if (!!item) {
    ...
    hanleStopAudio () {
      ...
      let terminalCode = this.roomSelectList
        ...
        .join(",");
      this.$parent.sendWebsocket(`{maindevno:"${controlCode}",devno:"${terminalCode}",type:"300",msg:"1"}`);
      let params = {
        controlId: uni.getStorageSync("controlInfo").id,
        type: "300",
    ...
    async setDynamicInfo (params) {
      let res = await Api.apiCall("post", Api.index.setDynamicInfo, JSON.stringify(params));
      if (res.state.code == "200") {
        // 刷新动态信息
        this.getDynamicInfo();
      } else {
    ...
    async updateGroup () {
      ...
      this.audioSelectList.forEach(item => {
        this.tableList[this.currentPage].map(i => {
          if (i.site == item.site) {
            i.startTime = this.week ? `每周${this.week} ${this.startTime.split(' ')[1]}` : this.startTime || '-';
            i.endTime = this.week ? `每周${this.week} ${this.endTime.split(' ')[1]}` : this.endTime || '-';
          }
          return i;
        });
      });
